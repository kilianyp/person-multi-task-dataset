...
from models.backbone.resnet import BatchnormBackbone, Bottleneck
...
@register_model("fpn_bn")
class FpnBatchnorm(BatchnormBackbone):
    def __init__(self, block, layers, stride, output_dim=256):
        super().__init__(block, layers, stride)
        self.output_dim = output_dim
        self.num_stages = 4  # pyramid stages
        # Top layer
        self.toplayer = nn.Conv2d(2048, output_dim, kernel_size=1, stride=1, padding=0)  # Reduce channels
    ...
    def build(cfg):
        ...
        # backwards compability
        output_dim = cfg['output_dim']
        model = FpnBatchnorm(Bottleneck, [3, 4, 6, 3], stride, output_dim)
        skips = ['fc']
        return model, skips, []
